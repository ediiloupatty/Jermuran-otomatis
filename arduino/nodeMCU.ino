...
const int RAIN_THRESHOLD = 400; // Nilai dibawah 400 berarti hujan

// Servo rotation time (in milliseconds)
const unsigned long ROTATION_TIME = 5000; // 5 detik
...
String clotheslineStatus = "Mendeteksi...";
...
void updateDisplay(int ldr, int rain, bool dark, bool raining)
{
    ...
    if (isRotating)
    {
        unsigned long remainingSecs = (ROTATION_TIME - (millis() - rotationStartTime)) / 1000;
...
void sendSerialData(int ldr, int rain, bool dark, bool raining)
{
    ...
    if (isRotating)
    {
        unsigned long remainingTime = (ROTATION_TIME - (millis() - rotationStartTime)) / 1000;
